...
`.env` is ignored by Git. Production values belong in the deployment platform's
...
An empty allowlist is valid. Required service values are validated independently, so a
worker does not need Discord secrets and a bot does not need OAuth client credentials.
Validation failures stop startup and list field names without echoing values.

## `config.json`

`config.json` is committed, contains no secrets, and is strictly validated. Unknown
keys are rejected.

- `application`: branding, canonical domains, owner display metadata, locales.
- `discord`: command modes, guild registration targets, permissions, intents,
  partials, and sharding strategy.
...
- `security`: rate limits, session/grant lifetime, audit retention, signed-request age,
  and maximum pagination.
...
Create an optional `config.development.json`, `config.test.json`, or
`config.production.json` at the repository root. Objects are recursively merged;
arrays and scalar values replace the base value.
...
Do not place tokens, passwords, private hosts, or encryption keys in overrides. After
editing configuration, run `pnpm typecheck` and start the relevant service. The owner
command `/admin reload-config` reloads and validates process-local non-secret config;
it is audited, but does not mutate environment variables.

## Operational changes

Changes to auth origins, CORS, cookies, invite permissions, intents, timeouts, pool
capacity, retention, or sharding require a deployment review. Roll out command
registration changes first to a development guild; Discord global propagation can take
time.
