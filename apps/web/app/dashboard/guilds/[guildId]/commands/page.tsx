    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
      <Card>
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          Overrides complement runtime Discord permission checks; they never bypass required bot or user permissions.
        </p>
          ...
          {overrides.length === 0 ? (
            ...
            overrides.map((override) => (
              <div key={override.id} className="grid gap-2 py-4 text-sm sm:grid-cols-3">
                ...
                <span>{override.subjectType.toLowerCase()} {override.subjectId}</span>
      ...
      <Card>
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">Discord role IDs are stored as snowflake strings.</p>
        <ActionForm
        ...
        >
          ...
          <label className="grid gap-2 text-sm font-medium">
            Command
            <select name="commandName" className="h-11 rounded-xl border bg-[var(--background)] px-3">
              {builtInModules.flatMap((module) => module.commands).map((command) => (
          ...
          <label className="grid gap-2 text-sm font-medium">
            Discord role ID
            <input name="roleId" pattern="\d{17,20}" required className="h-11 rounded-xl border bg-[var(--background)] px-3" />
          </label>
          <label className="grid gap-2 text-sm font-medium">
            Effect
