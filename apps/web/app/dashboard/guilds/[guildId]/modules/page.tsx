        ...
        return (
          <Card key={module.metadata.key}>
            <div className="flex items-start justify-between gap-4">
              ...
              <span
                className={`rounded-full px-2.5 py-1 text-xs font-semibold ${enabled ? 'bg-emerald-500/10 text-emerald-500' : 'bg-[var(--surface-strong)] text-[var(--muted)]'}`}
              >
                {enabled ? 'Enabled' : 'Disabled'}
              </span>
            </div>
            <p className="mt-6 text-xs text-[var(--muted)]">
              {module.commands.length} command(s) · cache segments{' '}
              {module.cacheInvalidation.segments.join(', ')}
            </p>
            <div className="mt-6">
              <ModuleForm
                guildId={guildId}
