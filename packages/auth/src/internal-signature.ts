import { AuthorizationError, constantTimeEqual, hmacSha256, sha256 } from '@sufbot/shared';
...
export const verifyInternalRequest = async (
  ...
  if (!Number.isFinite(timestamp) || Math.abs(now - timestamp) > options.maxAgeSeconds * 1000) {
    throw new AuthorizationError(
      'Internal request timestamp is invalid.',
  ...
  const expected = signInternalRequest(secret, method, path, body, fields.timestamp, fields.nonce);
  ...
  if (!(await replayStore.claim(fields.nonce, options.maxAgeSeconds * 2))) {
    throw new AuthorizationError(
      'Internal request replay was rejected.',
